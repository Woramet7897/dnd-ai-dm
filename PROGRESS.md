...
Last updated: 2026-07-24 (Phase 4 polish fixes — slug collision, flat dice, resolve_round)
...
| 4 - combat_manager | DONE | combat_manager.py | Conditions (5): apply_condition/tick_conditions per Section 20. Initiative with DEX-mod + tiebreak. start_combat idempotency guard (spec 9b-iv). resolve_attack with adv/disadv from conditions, crit (double dice), fumble, applies_condition from static catalog only. resolve_enemy_turn (first living target), resolve_companion_turn (lowest-HP target), both skip when stunned. classify_round_significance, build_routine_summary (template bank), build_round_narration_block (spec 9b-iii format). check_combat_end (victory/defeat). resolve_round(): full round orchestration in turn_order sequence, appends to round_log, ticks conditions, increments round, returns narration_block + combat_outcome. Polish fixes: (a) quest slug id collision deduplication in state_manager.apply_quest_updates (appends _2/_3 suffix until unique); (b) roll_dice() extended to accept flat-integer expressions like "1+0" and "5" (no NdX term); (c) _player_attacks Unarmed Strike damage="1+0" now takes real code path. DoD: 125/125 tests + 45/45 Phase 3 regression. |
