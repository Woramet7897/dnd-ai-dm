...
Last updated: 2026-07-22 (Phase 3 DONE)
...
| 1 - character_creator + validation | DONE | character_creator.py, validation.py | DEV-1: derive_stats() hardcodes Persuasion+Performance for classes with "Any" skill pool — fix at Phase 7: accept player skill choices as a parameter from the app.py multiselect widget. DEV-2: validate_extraction_output() passes world_updates.new_location through structurally (is-dict check only) — RESOLVED Phase 3: validation.py now calls dungeon_manager.validate_new_location() for that field. DEV-3: quest_updates also passes through with is-dict check only (same position as DEV-2 was) — deep schema validation deferred to dungeon_manager Phase 3 write path. DoD: 38/38 tests passed post-fix, exit code 0. |
| 2 - state_manager core | DONE | state_manager.py (overwrote v1) | DEV-1: add_quest via apply_state_updates() defaults to side quest category — main quest routing deferred to dungeon_manager Phase 3. DEV-2: _roll_d20 is a module-level function (not injected) to allow test monkeypatching; production code calls it by name inside resolve_check(). DoD: 73/73 tests passed, exit code 0. |
...
All phases up to 3 are DONE. Next: Phase 4 (combat_manager.py).
